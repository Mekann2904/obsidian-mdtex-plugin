...
tags: [クイックスタート, インストール, チュートリアル]
related: [docs/features.md, docs/configuration.md, docs/troubleshooting.md]
---

# クイックスタートガイド

[ドキュメントインデックス](./index.md) > クイックスタートガイド

## 概要

このガイドは、MdTeX プラグインを初めて使う方向けに、最短で PDF を生成する手順を説明する。
...
- **Obsidian**（デスクトップ版）- モバイル版には対応していない
- **Pandoc** - Markdown変換エンジン
- **LuaLaTeX** - PDF生成エンジン（TeX LiveまたはMacTeXに含まれる）
...
両方でバージョン情報が表示されれば、インストールは成功である。
...
3. 数秒から数十秒で PDF が生成される。
4. PDF は出力先（既定は Vault ルート）に保存される。

### ステップ3: 結果を確認

生成された PDF を開き、次を確認する。
- 日本語が正しく表示される
- 数式がレンダリングされる
- コードブロックにシンタックスハイライトが付く
...
**対処**:
デフォルトプリアンブルには日本語対応が含まれている。カスタムプリアンブルを使用する場合は、以下を含める：
