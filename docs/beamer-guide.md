...
MdTeXを使用してプレゼンテーションPDF（Beamer）を作成する方法を解説する。
...
BeamerはLaTeXのプレゼンテーション作成クラスである。PDF形式のスライドを生成し、以下の特徴がある：
...
Beamerでは、見出しレベル1（`#`）がフレーム（スライド）のタイトルになる。
...
段階的に表示するには、Pandocの`.incremental`クラスを使用する：
...
プラグインに同梱の[sample-beamer.md](../sample-beamer.md)を参照すること。
