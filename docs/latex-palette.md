...
tags: [LaTeX, コマンドパレット, 補完]
related: [docs/configuration.md, docs/features.md]
---

# LaTeXコマンドパレット

[ドキュメントインデックス](./index.md) > LaTeXコマンドパレット

## 概要

LaTeXコマンドパレットは、よく使うLaTeXコマンドを検索、挿入する機能である。インライン補完やゴーストテキスト機能も提供する。

---

## 機能概要

LaTeXコマンドパレットは以下の3つの機能を提供する：
...
2. **ショートカット**
   - カスタムショートカットを設定できる（Obsidianの設定 → ショートカット）

### 使用方法

1. コマンドパレットが開くと、利用可能なコマンドの一覧が表示される
...
設定画面で「LaTeXパレットとインライン補完を有効」をオン/オフできる。

---

## ゴーストテキスト補完

入力中に候補を薄い文字で表示し、素早く確定できる。
...
設定画面で「インラインゴースト補完を有効（実験的）」をオン/オフできる。
