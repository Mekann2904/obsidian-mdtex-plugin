...
テンプレートパックは、**`defaults.yaml` を含むフォルダ** である。
1 つのフォルダがそのまま 1 つの文書テンプレートになる。
...
- **必須ファイルは `defaults.yaml` だけ**である。これが含まれるフォルダだけが
  ドロップダウンに現れる。
- README.md / SKILL.md のようにフォルダ直下のファイルはパック扱いされない。
